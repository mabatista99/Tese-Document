...
It starts by addressing how \gls{IoT} impacts monitoring and surveillance and how it is
...
This figure also shows why smart-city monitoring is challenging. A single
platform may need to combine data from cameras, traffic sensors,
environmental sensors, and other devices that use different technologies and
produce different data formats. To support anomaly detection and operational
response, the platform must process this heterogeneous data and help coordinate
actions such as traffic regulation, incident handling, and public-space
monitoring.
...
temperature, humidity, soil analysis, and illumination. At the same time, the
agricultural area is being surveilled through a camera \cite{hengko_smart_agriculture}.
These measurements help assess plant well-being and soil state, making it
easier to decide when irrigation, ventilation, shading, or other interventions
are needed.
...
Lastly, deep learning models use neural networks with multiple layers
...
Selecting the appropriate framework is important for implementing these models
...
Another problem is that in the beginning, there is a low amount of data to
train the model.

Lastly, there is the need to adapt to model drift as sensor behavior
changes over time.
...
more complex ones that can be relevant for IoT, such as microservices,
...
building the best architecture for reliable IoT-system monitoring and anomaly
detection systems.
...
compared with other modular architectures. Suppose that hypothetically,
...
usage but does not require scaling, and the third is barely used and
...
Every change in the codebase also requires the whole application to be
...
its own technology stack, which can simplify the development of some specific
...
The structure of a microservices application is depicted in Figure~\ref{fig:architectures:microservices}.
...
consumer can be added and subscribe to existing events. This does not require
any change in the producer in order to work, which makes it easier to extend
...
only when needed. These functions are triggered
...
Another advantage is that since the developers do not need to handle all the infrastructure management,
...
Since there is no need to access a server, it allows for real-time decisions
in systems like self-driving cars or industrial automation. When there is
...
It is also a reliable choice because it can operate offline by storing data
locally until the connection is restored \cite{MicrosoftEdgeComputing}.

It also enhances data privacy, as sensitive information, such as biometric data,
...
its infrastructure is very simple, and it is relatively fast to have an initial
functional product. This architecture is often used as a starting point, and
eventually, as the application grows, it is migrated to more complex architectures.
...
systems. It is widely used in bigger e-commerce applications where the application
...
responsiveness is crucial. It is widely used in \gls{IoT} systems, in
...
real-time processing. It is widely used in \gls{IoT} systems in industrial
automation or autonomous vehicles, where real-time decisions are critical.

It is also common to mix different architectures in the same system using
...
designed to simplify server-side development. It is characterized by being
fast and unopinionated, which means that it is flexible in the way you implement
things.

In Express, it is possible to stack functions (middlewares) to handle \gls{HTTP}
requests sequentially, allowing custom logic to be easily integrated.
Routing using this framework is straightforward. It is very simple to define
...
this framework lacks built-in features. Express does not provide features like
...
A typical use case for NestJS is a TypeScript backend that exposes \gls{REST}
\gls{API}s while also consuming events from technologies such as Kafka or
\gls{MQTT} in a modular microservice architecture.

\subsubsection{Java Spring}
Spring is the most popular framework for building complex and robust
applications using Java. It is a great solution for developing scalable and secure
applications. It is one of the most complete backend frameworks, having features
...
design.
It is a very flexible solution handling every type of architecture, from
...
Boot.
Another downside of this framework is that it is a heavy framework and
requires higher resources to run when compared with other lightweight solutions.

A typical use case for Spring is an enterprise backend that must combine
\gls{REST} \gls{API}s, strong security controls, and integration with several
internal services.

\subsubsection{ASP.NET Core}
ASP.NET Core is an open-source framework developed and maintained by Microsoft.
It is a high-performance framework designed for scalability and modularity and
...
Despite being mainly a backend framework, it is also possible to develop the
...
Setting up the middleware pipeline and dependency injection can also be a complex
process.

This framework can be a great choice for large-scale systems but overwhelming
for smaller projects.

A typical use case for ASP.NET Core is a high-performance web backend in an
organization that already relies on the Microsoft ecosystem and needs strong
support for authentication, middleware, and scalable deployment.

Table~\ref{tab:backend-framework-synthesis} summarizes the main
characteristics of the backend frameworks discussed above.

\begin{table}[htbp]
	\centering
	\caption{Synthesis of backend framework characteristics.}
	\label{tab:backend-framework-synthesis}
	\renewcommand{\arraystretch}{1.15}
	\footnotesize
	\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{1.9cm} >{\raggedright\arraybackslash}p{2.1cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{2.6cm} >{\raggedright\arraybackslash}p{2.7cm}}
		\toprule
		\textbf{Framework} & \textbf{Typical Ecosystem} & \textbf{Main Strengths} & \textbf{Main Trade-Off} & \textbf{Best Fit} \\
		\midrule
		Express & Node.js and JavaScript & Lightweight routing, flexible middleware composition, and low entry barrier & Few built-in features and limited architectural guidance & Small-to-medium backends, prototypes, and custom \gls{REST} \gls{API}s \\
		NestJS & Node.js and TypeScript & Strong modular structure, dependency injection, and native support for event-driven and microservice patterns & More opinionated and heavier than simpler Node.js frameworks & TypeScript backends with clear module boundaries and broker integration \\
		Spring & Java ecosystem & Mature ecosystem, strong security support, rich integration options, and modular design & Higher complexity, steeper learning curve, and higher resource use & Enterprise services with strict security and integration requirements \\
		ASP.NET Core & .NET ecosystem & High performance, strong tooling, modular middleware pipeline, and built-in security support & Can be complex for small systems with limited scope & Scalable web platforms in Microsoft-oriented environments \\
		\bottomrule
	\end{tabularx}
\end{table}

In this context, framework selection should be guided mainly by the system
requirements. The main criteria are the intended architecture, the level of
modularity required, the availability of built-in support for security and
inter-service communication, the expected system scale, and the desired balance
between flexibility and structure. Express is better suited to simpler
backends or cases where the architecture is assembled manually. NestJS is more
appropriate when a TypeScript system benefits from clear module boundaries and
support for event-driven communication. Spring and ASP.NET Core fit better in
larger systems that require mature security features, stronger integration
support, and more complete application frameworks. As a secondary practical
factor, prior knowledge of the language and ecosystem can also reduce
implementation effort and development risk.
...
It is commonly used in resource-constrained systems like IoT systems because
it is easy to implement and requires low bandwidth \cite{Jayapal2019, CommunicationProtocols}.
...
more complex systems, providing reliable message queuing and
...
environments, such as \gls{IoT} systems. It is built on \gls{UDP} instead of \gls{TCP},
...
The backend is where all the business logic relies, and it is what provides the frontend
the data for it to display to the end user. Effective communication between
these two parts is important for a well-designed system.
\subsubsection{Protocols}
...
responds. It is a stateless, meaning each request is independent and does not
affect the others. It is commonly used for fetching \gls{HTML}, images, videos,
...
WebSocket connection. Unlike \gls{HTTP}, it does not rely on requests, and both
...
independent packets. It does not guarantee delivery, order, or error
...
each resource is independent, meaning that it does not depend on other
...
It is commonly used in microservices architectures and real-time applications \cite{newton2024benchmarking}.

Despite \gls{SOAP} being defined as a protocol, it is more comparable to \gls{API}
...
Security is an essential part of any system because it protects both the data
and the system itself. When designing security, several aspects need to be
addressed.

One very common example is the authentication process.
It is important that the component that does the request identify itself
...
access token. The refresh token is valid for a longer period of time (typically
one week).
...
Besides data encryption, it is also important to check data integrity.
To ensure that data is not changed during transmission, techniques like
\gls{HMAC} can be used \cite{schiller2022landscape}.

It is also important to control who can access each data.
...
prevent access to sensitive data from users that cannot access it.
...
attacks.
\gls{CSP} can be used to prevent \gls{XSS} attacks. For \gls{DoS}, it is possible to enforce
rate limits and \gls{API} quotas. To prevent \gls{SQL} Injection, strategies like
input sanitization or the use of an \gls{ORM} are essential \cite{schiller2022landscape}.
...
consistency, integrity, and reliability. They also provide a way of
...
Relational databases also benefit from maturity, with mature tools available for backup, replication, and access control, such as PostgreSQL and MySQL.
...
\gls{NoSQL} databases do not support complex queries with features like JOINs,
...
There is also the problem of data duplication.
...
ideal when the system requires \gls{ACID} compliance. It is also a good choice for
...
data, where flexibility is a key requirement.
...
context, can also be a great choice for storing unstructured data that is not
...
Table~\ref{tab:database-synthesis} summarizes the main trade-offs of the
database families discussed in this section.

\begin{table}[htbp]
	\centering
	\caption{Synthesis of database families for \gls{IoT} systems.}
	\label{tab:database-synthesis}
	\renewcommand{\arraystretch}{1.15}
	\footnotesize
	\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.1cm} >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}p{1.9cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{2.4cm} >{\raggedright\arraybackslash}p{2.4cm}}
		\toprule
		\textbf{Database Family} & \textbf{Schema Model} & \textbf{Scalability} & \textbf{Query Strengths} & \textbf{Main Trade-Off} & \textbf{Best IoT Use} \\
		\midrule
		Relational & Fixed schema with tables and relations & Mostly vertical, with more complex horizontal scaling & Complex joins, transactional consistency, and structured queries & Rigid schema and harder scaling at very high volume & Users, devices, permissions, rules, and other structured metadata \\
		NoSQL & Flexible documents, columns, graphs, or key-value pairs & Strong horizontal scaling & Fast access to semi-structured data and flexible records & Weaker consistency guarantees and limited relational queries & Flexible event records, cached views, and non-relational data \\
		Time-Series & Timestamp-indexed records & High write scalability for telemetry workloads & Time-window queries, aggregations, and monitoring metrics & Less suitable for rich relational models and non-temporal data & Sensor telemetry, metrics, and high-frequency monitoring data \\
		\bottomrule
	\end{tabularx}
\end{table}

Database selection should follow the data shape and the expected access
patterns. In the context of this thesis, relational databases are the best fit
for structured metadata and access control, time-series databases are the best
fit for telemetry, and \gls{NoSQL} databases are useful for flexible
non-time-based records. For many \gls{IoT} systems, the most adequate solution
is therefore a hybrid one, with each database family being used where its
...
With the advance of technology, many libraries were created to simplify the
...
simplifies the development of complex and dynamic user interfaces. It is the most widely
...
Each component has its own logic and styling, and it is declarative, i.e., the
...
One of ReactJS problems is that it does not have built-in support for routing,
...
\gls{CSR}, used by ReactJS, the server sends to the client a small
...
\gls{HTML} page and does not take into account the dynamic \gls{HTML} created after by
...
\gls{ETL} is a workflow that transforms raw data into data that is ready to be
stored and analyzed. \gls{IoT} systems use \gls{ETL} pipelines to process and store data from sensors,
smart devices, and edge computing nodes. Figure \ref{fig:etl:pipeline}
illustrates the \gls{ETL} pipeline, with the data being extracted from the devices,
...
The extract stage collects raw data from \gls{IoT} devices, \gls{API}s, cloud
platforms, or edge systems. This can be done through \gls{MQTT} brokers,
\gls{HTTP} or \gls{CoAP} endpoints, Apache Kafka streams, or \gls{IoT}
gateways.

After collection, the data is transformed through processes such as cleaning,
filtering, and enrichment. Cleaning removes noise, duplicates, and null
values. Filtering discards data that is not relevant for the target analysis.
For example, if the goal is to detect high temperatures, the pipeline can
discard measurements that remain within the normal range. The data can also be
enriched with metadata such as timestamps and device location
\cite{singu2022etl}. This stage can run on the edge, in the cloud, or in a
hybrid setup. Tools such as Apache NiFi and \gls{AWS} \gls{IoT} Analytics
support this kind of transformation.

In the last stage, the transformed data is loaded into the systems that
support the application, such as relational databases, time-series databases,
object stores, or analytical repositories. This stage influences how
efficiently the data can later be queried, aggregated, and exposed to
dashboards or analytics services.
...
In practical terms, \gls{BI} connects data collection, storage, analysis, and
presentation so that raw telemetry becomes information that operators can
inspect and act on. It is not limited to dashboards. It also includes the
processes that prepare the data for comparison, trend analysis, and operational
decision-making.
...
These categories are complementary. Descriptive and diagnostic analytics help
explain what has already happened, predictive and prescriptive analytics
support future decisions, and real-time analytics supports immediate
operational response. In monitoring systems, the value of \gls{BI} often comes
from combining these perspectives instead of relying on only one of them.
...
IoT monitoring platforms have different approaches on how to connect devices, route telemetry,
and expose data to downstream applications. Cloud platforms usually provide scalable solutions with analytics, and edge
services. Open-source platforms offer more control over deployment and data
ownership, but usually require more integration work. This section reviews four solutions, AWS IoT Core, Microsoft Azure IoT Hub, ThingsBoard,
and FIWARE, and highlights the trade-offs that are most relevant to this thesis.

\subsection{AWS IoT Core}
AWS IoT Core is a managed cloud platform designed for large-scale deployments.
It combines secure device connectivity with message routing and
integration with analytics, storage, and edge services. This makes it a strong
option for enterprise environments that already rely on AWS and need a platform
that can scale with high device and message volumes. At the same time, that
strength comes from a broad ecosystem of services and configuration choices,
which increases platform coupling and operational complexity. In practice, AWS
IoT Core is well suited to stable industrial deployments, but it is less
attractive when the goal is to build a lightweight monitoring pipeline
\cite{panagou2025comprehensive, s22062196, article5}.

\subsection{Microsoft Azure IoT Hub}
Microsoft Azure IoT Hub follows a similar cloud-centered strategy. It provides
bi-directional communication, device management, and integration with Azure
services for analytics, storage, and edge processing. This modular structure is
useful in enterprise settings, especially when the solution must fit into a
larger Microsoft ecosystem. However, the same modularity increases setup effort
and makes the final architecture depend on several coordinated services. As
result, Azure IoT Hub offers scalability and flexibility, but not the simplest path
for a focused monitoring platform that prioritizes a small and adaptable
ingestion stack \cite{panagou2025comprehensive, s22062196}.

\subsection{ThingsBoard}
ThingsBoard is an open-source IoT platform that combines device management,
data collection, processing, and visualization in the same system. It supports
multiple protocols and provides dashboards and rule-based processing out of the
box, which makes it attractive for self-hosted deployments and rapid
development. This is also its main trade-off. ThingsBoard covers many
concerns at once, from transport integration to user-facing visualization,
which is useful for general IoT solutions but broader than needed for a more
focused monitoring platform. For this thesis, it is an important reference
because it shows the advantages of an integrated open-source platform, while
also showing the cost of carrying features that are outside the main ingestion
and processing path \cite{app12042172}.

\subsection{FIWARE}
FIWARE takes a different approach. Instead of offering a single vertically
integrated platform, it defines a set of open standards and reusable
components for context data management, with the context broker model at its
core. This makes FIWARE especially strong in interoperability, integration with
external services, and cross-domain scenarios where several systems need to
share the same context. The trade-off is that a complete
solution usually requires assembling multiple components, which gives more
flexibility but also increases the effort. In the context of this
thesis, FIWARE is a relevant reference because it represents the opposite end
of the spectrum from a tightly scoped platform: it is open, extensible, and
powerful, but heavier to configure for a focused telemetry monitoring solution
\cite{app12042172}.

\subsection{Related Work Synthesis}
These platforms show two recurring trade-offs. AWS IoT Core and Azure IoT Hub
provide scale and managed services, but they also tie the solution closely to
their cloud ecosystems. ThingsBoard and FIWARE provide more deployment control,
but they either combine many concerns in one platform or require more
integration work. None of them is primarily designed for a lightweight,
event-driven monitoring platform that accepts heterogeneous device data,
transforms it with limited schema assumptions, and exposes it to a custom
application stack. This gap motivates the platform proposed in this thesis \cite{panagou2025comprehensive, app12042172}.

The proposed approach addresses the gaps identified in this synthesis, namely
the need for a lightweight event-driven monitoring platform that accepts
heterogeneous device data, supports extensibility, keeps deployment under the
developer's control, and continues operating during temporary connectivity
loss.

Table~\ref{tab:related-work-comparison} summarizes the same discussion using
the five design goals adopted in this thesis: genericity, real-time
processing, extensibility, security, and offline resilience. In this context,
genericity refers to accepting heterogeneous device data with limited schema
assumptions. Security and offline resilience are included because they are
explicit design goals of this thesis, even when they are not the main focus of
every platform reviewed. In this table, \textit{Strong} means that the goal is
central to the platform design, \textit{Partial} means that it is supported but
not as a defining property, and \textit{Weak} means that it is outside the
main focus of the platform.

\begin{table}[htbp]
	\centering
	\caption{Comparison of related platforms against the main design goals of this thesis.}
	...
	\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.8cm} >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X}
		\toprule
		\textbf{Platform}       & \textbf{Genericity} & \textbf{Real-Time Processing} & \textbf{Extensibility} & \textbf{Security} & \textbf{Offline Resilience} \\
		\midrule
		AWS IoT Core            & Partial             & Strong                        & Strong                 & Strong            & Partial                     \\
		Microsoft Azure IoT Hub & Partial             & Strong                        & Strong                 & Strong            & Partial                     \\
		ThingsBoard             & Strong              & Strong                        & Strong                 & Partial           & Strong                      \\
		FIWARE                  & Strong              & Partial                       & Strong                 & Partial           & Weak                        \\
		Proposed platform       & Strong              & Strong                        & Strong                 & Strong            & Strong                      \\
		\bottomrule
	\end{tabularx}
\end{table}
