...
The same repository also contains the edge node under a separate top-level application. It is another NestJS application, with Mosquitto instead of EMQX, SQLite instead of TimescaleDB, and no Kafka. Keeping it in its own application tree avoids mixing edge-specific runtime concerns into the cloud services while still allowing shared development in one codebase.

\subsection{Deployment and Orchestration}
Docker Compose manages the entire platform with twenty-three containers on a shared network, so they find each other by container name. Six of these are infrastructure dependencies: EMQX, Kafka, TimescaleDB, Redis, MinIO, and Mailhog (for development email testing). Eleven are application and operational containers: the nine backend services, the \gls{MQTT}-Kafka bridge, and a Kafka \gls{UI} for broker inspection. The remaining six handle monitoring: Prometheus, Grafana, cAdvisor, and exporters for Kafka, Redis, and PostgreSQL. The overall deployment topology is shown in Figure~\ref{fig:deployment-topology}.
