...
The Authentication Service handles identity for users and devices. It issues \gls{JWT} tokens and acts as the source of truth for authentication across the platform entry points. User registration and login use email and password credentials. Passwords are hashed with bcrypt before being stored. For web users, the service issues a 15-minute access token and a 30-day refresh token. For devices, it issues a 1-day access token and a 30-day refresh token. All tokens are signed with RS256 (Section~\ref{subsec:jwt-rs256}).
...
The \gls{MQTT}-Kafka bridge is a standalone Go service that subscribes to the broker and publishes to Kafka. For raw device telemetry, it extracts the device identifier from the \gls{MQTT} topic, parses the \gls{JSON} payload, and wraps it in an envelope with the device \gls{ID}, timestamp, and payload fields before producing it to Kafka. The same device identifier is used as the Kafka partition key, so messages from the same device stay in the same partition and keep their order. Edge telemetry and edge alerts are forwarded without transformation because the edge node already sends normalized data.

The bridge uses a buffered channel with a capacity of 10\,000 messages and a pool of four worker goroutines that drain the channel in parallel. Each worker accumulates messages into batches of up to 2\,000 and flushes every 50\,ms, whichever comes first. All Kafka writes use Snappy compression. If a produce fails, the worker retries up to five times with a fixed one-second delay between attempts. On shutdown, the bridge drains the buffered channel before exiting.
...
The Ingestion Service is responsible only for turning incoming telemetry into the platform's normalized event format. It consumes from the \texttt{telemetry.raw} Kafka topic and normalizes each \gls{JSON} payload. It flattens the payload into individual key-value pairs and classifies each scalar value as number, boolean, or string. The flattening is recursive. Nested objects produce dot-separated keys, such as \texttt{sensor.temperature}, and arrays produce indexed keys, such as \texttt{readings.0.value}. One raw message can therefore produce multiple normalized events on \texttt{telemetry.events}, one for each data point.

All consumers in this service use Kafka batch processing. The main ingestion consumer handles each batch in three phases. First, it transforms all messages. Second, it produces the normalized events to Kafka in chunks of 1000, which keeps the batch under Kafka's maximum message size. These Kafka writes use Snappy compression and retry with exponential backoff. Third, it commits offsets and sends a heartbeat.

A \texttt{NormalizedConsumer} handles pre-normalized edge telemetry from the \texttt{telemetry.normalized} topic. It skips the flattening step and emits directly to \texttt{telemetry.events}. This keeps the edge path aligned with the main cloud path. Once the data is on \texttt{telemetry.events}, the Metadata, Storage, Analytics, and Read Services consume it independently.

When failures occur, the service uses a dead-letter topic to keep the pipeline moving. Malformed raw \gls{JSON} messages are logged and skipped. Transform failures and Kafka infrastructure failures are sent to the \texttt{telemetry.dlq} topic with an error type tag, either \texttt{TRANSFORM\_ERROR} or \texttt{INFRA\_ERROR}. An hourly cron job reads from the \gls{DLQ} and retries only infrastructure errors, because transform errors are deterministic and would fail again.

\subsection{Metadata Service}
\label{subsec:MetadataServiceImpl}

The Metadata Service is a dedicated Kafka consumer that keeps the \texttt{DataKey} catalogue aligned with the telemetry stream. It subscribes to \texttt{telemetry.events}, extracts the device identifier, key name, and inferred value type from each event, and upserts the corresponding data key record.

To reduce database overhead, the consumer first deduplicates device-key pairs within each Kafka batch. The service then applies an in-memory throttle of 30 seconds per device-key pair before issuing another upsert. This avoids repeated writes for hot telemetry fields while still refreshing the metadata when a device starts sending a new field or when a type change must be reflected in the catalogue. These discovered data keys are later used by the web application when users create rules and when device detail pages are shown.

\subsection{Storage Service}
\label{subsec:StorageServiceImpl}

The Storage Service owns the high-volume persistence path for both telemetry and alerts. For telemetry, it subscribes to \texttt{telemetry.events} with its own consumer group and writes each batch to the \texttt{DeviceData} hypertable in TimescaleDB. To reduce per-row overhead, the batch insert uses raw SQL with parameterized placeholders, also chunked at 1000 rows, instead of the \gls{ORM}'s insert method. If a batch insert fails, the service falls back to individual row inserts and sends any rows that still fail to a storage-specific \gls{DLQ} path.

For alerts, the same service subscribes to \texttt{alerts.events}. Each alert event already contains the snapshot fields needed for storage, including the rule context, threshold values, trigger value, target user, and alert type. The service writes that snapshot to the \texttt{Alert} table, so the stored record stays readable even if the original rule or detector configuration changes later.
...
When a rule or anomaly detector fires, the service builds an alert event that already includes the snapshot fields needed for persistence and delivery. These fields include the rule name, operator, threshold values, and trigger value, so the alert remains readable even if the originating rule is later deleted. The \texttt{PATCH /alerts/:id/acknowledge} endpoint allows users to mark alerts as reviewed. An \texttt{EdgeAlertsConsumer} subscribes to \texttt{alerts.from-edge} and republishes edge-generated alerts into the standard cloud alert stream. If emitting an alert to Kafka fails, the event goes to the \texttt{alerts.dlq} topic and a cron job retries it every minute, so temporary failures do not cause alerts to disappear.
